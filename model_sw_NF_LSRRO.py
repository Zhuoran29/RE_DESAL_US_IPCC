...
from fs_NF_LSRRO_simplified import fs_NF_LSRRO_crys_simple
...
def NF_LSRRO_cryst_cal():
    ...
    # csv_outfile3 = 'minLCOW_dry_ssp5_2050_PV_NF_LSRRO_cryst_sm18.csv' 
    # csv_outfile4 = 'county_dry_ssp5_2050_PV_NF_LSRRO_cryst_sm2.csv' 
    # csv_outfile5 = 'minLCOW_dry_ssp5_2050_PV_NF_LSRRO_cryst_sm24.csv'
    csv_outfile6 = 'coastal_2050_PV_NF_LSRRO_sm3.csv'

    # sensitivity file names
    # csv_outfile6 = 'sensitivity_low_battery.csv'
    ...
    for i in csv_writers:
        i.writerow(["County", "State_ID", "County_ID", "Latitude", "Longitude", 
                    ...
                    "LSRRO_recovery", "LSRRO_Capacity (m3/day)", 
    ...
    for index, row in df.iterrows():
        ...
        solute_dict = {
            ...
            'Ca': 412.3,
            'K': 399.1,
            'Cl': 19443.32499227845,
            'CO3': 0,
            'HCO3': 141.7,
            'SO4': 2712,
        }

        tds = sum(solute_dict.values())
        depth = 0
        ...
        # ADD DESAL
        m, lsrro, cryst, data_table, overall_performance, NF_brine_tds, capacities = fs_NF_LSRRO_crys_simple(
                                                                        desal_cap,
                                                                        solute_dict,
                                                                        tds)
        ...
        NF_permeate_tds = capacities[-2]
        SEEC_lsrro = -0.0017 * NF_permeate_tds**2 + 0.1859 * NF_permeate_tds + 0.6912 # kWh/m3
        ...
        latitude, longitude, tds, NF_brine_tds, depth, 
...
def solar_multiple_optimizer(
    ...
    for i in range(len(solar_multiples)):
        ...
        results = {"LCOW_no_grid": float("inf"), "Curtailment percentage (%)": 0, "LCOW": float("inf"), "Solar multiple": 1,  "Storage": 0, 
            ...
            "CAPEX LCOW ($/m3)": 0,"Fixed OPEX LCOW": 0, "OPEX LCOW ($/m3)": 0}

        PV_capacity = Elec_req * sm 
        ...
        for storage in storage_hours:
            ...
            feed_vol, nf_cap, lsrro_cap, cryst_cap, NF_permeate_tds, lsrro_rr = capacities
            ...
            LSRRO_capex = (118.41*NF_permeate_tds**2 + 2735.7*NF_permeate_tds + 14117) / 86 * lsrro_cap
            cryst_capex = 0
            drilling_cost = 650 * depth * desal_cap / 3785.4 / 3.26

            battery_cost = (197 * storage + battery_power_cost[storage]) * Elec_req
            ...
            LSRRO_fix_opex = LSRRO_capex/0.1*0.03
            cryst_fix_opex = 0
            ...
            total_capex = NF_capex + LSRRO_capex + cryst_capex + drilling_cost + battery_cost
            total_fix_opex = NF_fix_opex + LSRRO_fix_opex + cryst_fix_opex

            cost_elec = SEEC_avg *  lcoe_after_curtail
            cost_NaCl = salt_prod / water_prod * NaCl_rec

            LCOW_no_grid = ((total_capex * 0.1 
                + total_fix_opex) 
                / (water_prod * 0.9)
                + cost_th
                + cost_elec
            ...
            cost_elec = SEEC_avg * (grid_perc/100 * grid_rate + (1- grid_perc/100) * lcoe_after_curtail)
            cost_NaCl = 0
            ...
            capex_lcow = total_capex * 0.1 / ((lsrro_cap + cryst_cap * 0.753 ) * 365 * 0.9)
            fixed_opex_lcow = total_fix_opex / ((lsrro_cap + cryst_cap * 0.753 ) * 365 * 0.9)
            opex_lcow = (cost_th
                + cost_elec
                + cost_NaCl)

            # print(sm, storage, round(LCOW,4), round(LCOW_no_grid,4), round(cost_elec,3))
            # Update value if a lower LCOW is achieved
            if LCOW < minLCOW * 1.01:
                results = {"LCOW_no_grid": LCOW_no_grid, "Curtailment percentage (%)": curt_perc, "LCOW": LCOW, "Solar multiple": sm,  "Storage": storage, 
                            ...
                            "CAPEX LCOW ($/m3)": capex_lcow,"Fixed OPEX LCOW": fixed_opex_lcow, "OPEX LCOW ($/m3)": opex_lcow
                            }
                
            minLCOW = min(LCOW, minLCOW)
        ...
        capex_lcow = results["CAPEX LCOW ($/m3)"]
        fixed_opex_lcow = results["Fixed OPEX LCOW"]
        opex_lcow = results["OPEX LCOW ($/m3)"]

        (county, state_id, county_id, 
        latitude, longitude, brackish_tds, NF_brine_tds, depth, 
        brackish_avail, desal_cap, 
...
def simulation( gen, 
    ...
    for i in range(len(gen)):
        ...
        prod[i] = (load[i] )/ power_load * max_prod  
        salt[i] = (load[i] )/ power_load * salt_prod
