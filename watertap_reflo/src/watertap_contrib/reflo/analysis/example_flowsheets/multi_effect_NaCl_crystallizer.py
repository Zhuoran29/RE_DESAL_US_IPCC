...
from watertap_reflo.src.watertap_contrib.reflo.unit_models.zero_order.crystallizer_zo_watertap import (
    Crystallization,
)
import watertap_reflo.src.watertap_contrib.reflo.property_models.cryst_prop_pack as props
from watertap_reflo.src.watertap_contrib.reflo.costing import (
    TreatmentCosting,
...
def add_heat_exchanger_eff2(m):
    ...
    def heat_transfer_equation_eff_2(b, t):
        return b.fs.eff_1.energy_flow_superheated_vapor == pyunits.convert(
...
def add_heat_exchanger_eff3(m):
    ...
    def heat_transfer_equation_eff3(b, t):
        return eff_2.energy_flow_superheated_vapor == pyunits.convert(
...
def add_heat_exchanger_eff4(m):
    ...
    def heat_transfer_equation_eff4(b, t):
        return eff_3.energy_flow_superheated_vapor == pyunits.convert(
...
def multi_effect_crystallizer_initialization(m):
    ...
    m.fs.eff_1.initialize(outlvl = idaeslog.CRITICAL)
    m.fs.eff_2.initialize(outlvl = idaeslog.CRITICAL)
    m.fs.eff_3.initialize(outlvl = idaeslog.CRITICAL)
    m.fs.eff_4.initialize(outlvl = idaeslog.CRITICAL)
    ...
    # Touch variables
    m.fs.eff_1.properties_out[0].conc_mass_phase_comp["Liq", "NaCl"]
    m.fs.eff_2.properties_out[0].conc_mass_phase_comp["Liq", "NaCl"]
    m.fs.eff_3.properties_out[0].conc_mass_phase_comp["Liq", "NaCl"]
    m.fs.eff_4.properties_out[0].conc_mass_phase_comp["Liq", "NaCl"]
    m.fs.eff_1.properties_out[0].dens_mass_phase["Liq"]
    m.fs.eff_2.properties_out[0].dens_mass_phase["Liq"]
    m.fs.eff_3.properties_out[0].dens_mass_phase["Liq"]
    m.fs.eff_4.properties_out[0].dens_mass_phase["Liq"]
    m.fs.eff_1.properties_in[0].dens_mass_phase["Liq"]
    m.fs.eff_2.properties_in[0].dens_mass_phase["Liq"]
    m.fs.eff_3.properties_in[0].dens_mass_phase["Liq"]
    m.fs.eff_4.properties_in[0].dens_mass_phase["Liq"]
...
def get_model_performance(m):
    ...
    feed_density = [
        i.properties_in[0].dens_mass_phase["Liq"].value for i in effs
    ]
    ...
    overall_SEEC = value(pyunits.convert(
    (
    ...
    to_units=pyunits.kW,
    )) / (m.fs.eff_1.properties_in[0].flow_vol_phase["Liq"].value * 3600) # m3/h condensation
...
if __name__ == "__main__":
    ...
    eff_1,eff_2,eff_3,eff_4 = m.fs.eff_1,m.fs.eff_3,m.fs.eff_3,m.fs.eff_4
    ...
    def total_feed(b, t):
        ...
        return (
            ...
            == total_flow_rate / 86400 * pyunits.m**3 / pyunits.s
        )

    m.fs.props.set_default_scaling("flow_mass_phase_comp", 1e-1 / 10, index=("Liq", "H2O"))
    m.fs.props.set_default_scaling("flow_mass_phase_comp", 1e-1 / 10, index=("Liq", "NaCl"))  

    iscale.set_scaling_factor(eff_1.area, 1e-1 / 10)
    iscale.set_scaling_factor(eff_2.area, 1e-1 / 10)
    iscale.set_scaling_factor(eff_3.area, 1e-1 / 10)
    iscale.set_scaling_factor(eff_4.area, 1e-1 / 10)
    ...
    print('total capex: ', value(m.fs.costing.total_capital_cost))
    print('total opex: ', value(m.fs.costing.total_operating_cost))
    print('elec: ', value(        pyunits.convert(
            (
                m.fs.eff_1.magma_circulation_flow_vol
                * m.fs.eff_1.dens_mass_slurry
                * Constants.acceleration_gravity
