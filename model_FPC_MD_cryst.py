...
from MDB_cost_new import MDB_cost
...
def FPC_MD_cryst_cal():
    ...
    for i in csv_writers:
        i.writerow(["County", "State_ID", "County_ID", "Latitude", "Longitude", 
                    ...
                    "MD_Capacity (m3/day)", "Cryst capacity (m3/day)", "FPC_Capacity (kW)", "Solar multiple", "Storage/battery (hour)", 
                    ...
                    "MD CAPEX ($)", "Cryst CAPEX ($)", "Drilling cost ($)", "Cryst LCOW ($/m3 feed)"
                    ])
...
def solar_multiple_optimizer(MDB, 
    ...
    for i in range(len(solar_multiples)):
        ...
        for storage in storage_hours:
            ...
            print(sm, storage, round(LCOW,3), round(curt_perc,1), round(grid_perc,1), round(lcoh_after_curtail,3))
            print(cost_model.cost_sys/1000)
            print(cost_model.cost_th)
            print(cost_model.STEC, cost_model.coh, cost_model.sam_coh)
            print('')
        ...
        cryst_LCOW = value(cryst.fs.levelized_cost_of_feed_brine)
