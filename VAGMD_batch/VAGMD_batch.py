...
from SW_functions import SW_Density, SW_Psat, SW_LatentHeat, SW_SpcHeat
import numpy as np
import math
import pandas as pd
...
class VAGMD_batch(object):
    def __init__(self,
        ...
        FeedC_r= 12,  # Feed concentration (g/L)
    ...
    def design(self):
       ...
       # Selection of model and ocoling system
        ...
        self.output = np.array([[i for i in range(len(self.STEC))], self.tminute, self.V, self.AccVd, self.S, self.TEO, self.Ttank, self.PFlux, self.R, self.ThEnergy, self.CEnergy])
    ...
    def VAGMD_Models_NaCl(self, TEI_r, FFR_r, TCI_r, Sgl_r, k, Ttank):
        ...
        matfile = loadmat('/Users/zhuoranzhang/Documents/Github_repos/Modeling_code/VAGMD_batch/VAGMD_Models_NaCl.mat')
        ...
        ATml = 5 # ((TEI_r - TCO) - (TEO - TCI_r )) / (math.log((TEI_r - TCO)/(TEO - TCI_r)))
    ...
    def simulation(self, gen, storage, overall_performance):
        ...
        self.thermal_load = self.P_req + cryst_load# kWh per hour
        ...
        for i in range(len(gen)):
            ...
            salt[i] = (fuel[i]+load[i] )/ self.thermal_load * salt_prod
        ...
        simu_output.append({'Name':'Curtailed solar thermal energy','Value':max(0, (sum(gen) - sum(load)) / 1000000) ,'Unit':'GWh'})   
...
if __name__ == "__main__":
    test = VAGMD_batch(
        ...
        FeedC_r= 128,  # Feed concentration (g/L)
        V0     = 30,  # Initial batch volume (m3)
        RR     = 50,  # Recovery rate
        ...
        j = 'c', # cooling system: 'c' for closed, 'o' for open
        Ttank = 35, # Initial temeprature of the saline feed
        TCoolIn = 15, # Initial tmeeprature of the cooling water
        dt = 30, # Time step for the simulation (< 480 second)
        )
    output = test.design()
    for i in output:
        print(i)
