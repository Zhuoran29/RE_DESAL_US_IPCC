...
#                 GOR = 10.475,  # Gained output ratio
                # downtime = 0.1, # Yearly downtime of the plant (ratio)
                 ...
                 cost_storage = 232 , # Cost of thermal storage ($/kWh)
        ...
        self.Prod = Prod + cryst_water_prod * 365
        ...
        self.cryst = cryst
        self.cryst_elec = cryst_elec
        self.cryst_th = cryst_th
        self.HX_capex = HX_capex
        self.salt_prod = salt_prod
    ...
    def lcow(self):
        ...
        self.drilling_cost = self.well_cost * self.brackish_depth * self.Capacity / 3785.4 / 3.26

        
        m = self.cryst
        self.cryst_CAPEX = value(m.fs.costing.total_capital_cost)
        self.CAPEX = ( (self.cost_sys*1000 + self.drilling_cost + self.cryst_CAPEX + self.HX_capex)*self.int_rate*(1+self.int_rate)**self.yrs) / ((1+self.int_rate)**self.yrs-1) / (self.Prod) 
        self.pump_brackish = self.brackish_depth * 0.3048 * 9.8 * 1000 * 2.778 * 1e-7 / self.RR / 0.85# kWh/m3 
        self.cost_elec = (self.SEEC + self.pump_brackish + self.cryst_elec * (1 - self.RR)) * self.coe 
        ...
        self.cost_th = (self.STEC + self.cryst_th * (1 - self.RR)) * (self.fuel_usage * self.coh + (1-self.fuel_usage) * self.sam_coh)
        self.salt_recovery = self.salt_cost * self.salt_prod / self.Prod
        self.OPEX = self.cost_elec + self.cost_th + self.cost_module_re + self.maintenancecost + self.insurancecost + self.operationalcost + self.salt_recovery
