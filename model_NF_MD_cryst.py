...
def FPC_MD_cryst_cal():
    ...
    for i in csv_writers:
        i.writerow(["County", "State_ID", "County_ID", "Latitude", "Longitude", 
                    ...
                    "NF_Capacity (m3/day)", "NF_p_TDS (g/L)", "NF_b_TDS (g/L)",
                    ...
                    "NF SEEC (kWh-e/m3)", "MDB SEEC (kWh-e/m3)", "MDB STEC (kWh-th/m3)", 
                    "Cryst SEEC (kWh-e/m3)", "Cryst STEC (kWh-th/m3)",  "Pumping energy (kWh-e/m3)",
                    "NF CAPEX ($)", "MDB CAPEX ($)", "Cryst CAPEX ($)", "Drilling cost ($)", 
    ...
    for index, row in df.iterrows():
        ...
        m1.fs.params = WaterParameterBlock(solute_list=[
                                                        "magnesium",
                                                        "potassium",
                                                        "sodium",
                                                        "sulfate",
        ...
        m1.fs.NF = NanofiltrationZO(property_package=m1.fs.params, database=m1.db)
        feed_flow_rate = 10

        m1.fs.NF.inlet.flow_mass_comp[0, "H2O"].fix(feed_flow_rate * 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "sulfate"].fix(solute_dict["SO4"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "sodium"].fix(solute_dict["Na"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "potassium"].fix(solute_dict["K"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "magnesium"].fix(solute_dict["Mg"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "carbonate"].fix(solute_dict["CO3"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "bicarbonate"].fix(solute_dict["HCO3"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "chloride"].fix(solute_dict["Cl"] * feed_flow_rate / 1000)
        m1.fs.NF.inlet.flow_mass_comp[0, "calcium"].fix(solute_dict["Ca"] * feed_flow_rate / 1000) 

        # Load parameters
        data = m1.db.get_unit_operation_parameters("nanofiltration")
        m1.fs.NF.load_parameters_from_database()
        m1.fs.NF.initialize()
        ...
        m1.fs.NF.eq_electroneutrality = Constraint(
            expr=0
            == sum(
                charge_comp[j]
                * m1.fs.NF.treated.flow_mass_comp[0, j] * 1000 / molar_mass[j]
                for j in charge_comp.keys()
            )
        )   

        constraint_scaling_transform(m1.fs.NF.eq_electroneutrality, 1e5)

        # Create TDS item for permeate and brine
        m1.fs.NF_treated_tds = Expression(
            expr=(sum(m1.fs.NF.treated.flow_mass_comp[0, j] for j in charge_comp) 
            / m1.fs.NF.properties_treated[0].flow_vol), doc="TDS of NF permeate"
        )
        m1.fs.NF_brine_tds = Expression(
            expr=(sum(m1.fs.NF.byproduct.flow_mass_comp[0, j] for j in charge_comp) 
            / m1.fs.NF.properties_byproduct[0].flow_vol), doc="TDS of NF brine"
        )

        try:
            m1.fs.NF.removal_frac_mass_comp[0, "chloride"].unfix()
            ...
            try:
                m1.fs.NF.removal_frac_mass_comp[0, "chloride"].fix(0.15)
                m1.fs.NF.removal_frac_mass_comp[0, "sodium"].unfix()

                results = solver.solve(m1)
                assert check_optimal_termination(results) 
            except:
                try:
                    m1.fs.NF.removal_frac_mass_comp[0, "sodium"].fix(0.1)
                    m1.fs.NF.removal_frac_mass_comp[0, "calcium"].unfix()
                    results = solver.solve(m1)
                    assert check_optimal_termination(results) 

                except:
                    try:
                        m1.fs.NF.removal_frac_mass_comp[0, "calcium"].fix(0.79)
                        m1.fs.NF.removal_frac_mass_comp[0, "sulfate"].unfix()
                        results = solver.solve(m1)
                        assert check_optimal_termination(results) 
                    except:
                        solute_dict["Na"] = tds / 58.5 * 23
                        solute_dict["Cl"] = tds / 58.5 * 35.5
                        m1.fs.NF.inlet.flow_mass_comp[0, "sulfate"].fix(1e-8)
                        m1.fs.NF.inlet.flow_mass_comp[0, "sodium"].fix(solute_dict["Na"] * feed_flow_rate / 1000)
                        m1.fs.NF.inlet.flow_mass_comp[0, "potassium"].fix(1e-8)
                        m1.fs.NF.inlet.flow_mass_comp[0, "magnesium"].fix(1e-8)
                        m1.fs.NF.inlet.flow_mass_comp[0, "carbonate"].fix(1e-8)
                        m1.fs.NF.inlet.flow_mass_comp[0, "bicarbonate"].fix(1e-8)
                        m1.fs.NF.inlet.flow_mass_comp[0, "chloride"].fix(solute_dict["Cl"] * feed_flow_rate / 1000)
                        m1.fs.NF.inlet.flow_mass_comp[0, "calcium"].fix(1e-8)

                        m1.fs.NF.removal_frac_mass_comp[0, "sulfate"].fix()
                        m1.fs.NF.removal_frac_mass_comp[0, "chloride"].unfix()
        ...
        NF_permeate_tds = value(pyunits.convert(m1.fs.NF_treated_tds, to_units = pyunits.kg/ pyunits.m**3))
        NF_brine_tds = value(pyunits.convert(m1.fs.NF_brine_tds, to_units = pyunits.kg/ pyunits.m**3))
        NF_permeate_vol = value(m1.fs.NF.properties_treated[0].flow_vol)


        # Add NF cost
        m1.fs.zo_costing = ZeroOrderCosting()
        m1.fs.NF.costing = UnitModelCostingBlock(flowsheet_costing_block=m1.fs.zo_costing)
        m1.fs.zo_costing.cost_process()
        ...
        SEEC_nf = value(m1.fs.zo_costing.aggregate_flow_electricity) / value(m1.fs.NF.properties_treated[0].flow_vol) / 3600

        # Pump energy
        brackish_avail = brackish * 3785.4 / (desal_cap / MDB_rr / 0.85) * 100
                                                                        #  NF RR # pump eff
        ...
        SEEC_avg = (MDB_cap * SEEC_MDB + cryst_prod * SEEC_cryst) / (cryst_prod + MDB_cap) + SEEC_nf + pump_energy_brackish
        ...
        latitude, longitude, tds, NF_permeate_tds, NF_brine_tds, depth, 
        ...
        solar_multiple_optimizer(MDB, 
                                ...
                                SEEC_avg,
                                STEC_avg,
                                [SEEC_nf, SEEC_MDB, SEEC_cryst, pump_energy_brackish],
                                [STEC_MDB, STEC_cryst],
                                MDB_cap,
                                cryst_cap,
                                nf_cap,
...
def solar_multiple_optimizer(MDB, 
    ...
    for i in range(len(solar_multiples)):
        ...
        results = {"Curtailment percentage (%)": 0, "LCOW": float("inf"), "Solar multiple": 1,  "Storage": 0, 
            ...
            "NF SEEC (kWh-e/m3)": 0, "MDB SEEC (kWh-e/m3)": 0, "MDB STEC (kWh-th/m3)": 0,
        ...
        SEEC_nf, SEEC_MDB, SEEC_cryst, pump_energy_brackish = SEECs
        ...
        for storage in storage_hours:
            ...
            #  CAPEX
            NF_capex = value(pyunits.convert(value(pyunits.convert(NF.fs.zo_costing.total_capital_cost, to_units=pyunits.USD_2018)) * pyunits.USD_2018, to_units = pyunits.USD_2021))  / ( value(NF.fs.NF.properties_treated[0].flow_vol) * 86400) * nf_cap
            ...
            # Fixed OPEX
            NF_fix_opex = value(pyunits.convert(value(pyunits.convert(NF.fs.zo_costing.total_fixed_operating_cost, to_units=pyunits.USD_2018 / pyunits.year)) * pyunits.USD_2018, to_units = pyunits.USD_2021)) / ( value(NF.fs.NF.properties_treated[0].flow_vol) * 86400) * nf_cap

            cryst_fix_opex = value(cryst.fs.costing.total_fixed_operating_cost)

            # variable OPEX
            external_lcoh = 0.03 # $/kWh
            NaCl_rec = -0.024 # $/kg
            ...
            total_capex = NF_capex + MDB_capex + cryst_capex + drilling_cost + TES_cost
            total_fix_opex = NF_fix_opex + cryst_fix_opex
            ...
            if LCOW < results["LCOW"]:
                results = {"Curtailment percentage (%)": curt_perc, "LCOW": LCOW, "Solar multiple": sm,  "Storage": storage, 
                            ...
                            "NF SEEC (kWh-e/m3)": SEEC_nf, "MDB SEEC (kWh-e/m3)": SEEC_MDB, "MDB STEC (kWh-th/m3)": STEC_MDB,
                            "Cryst SEEC (kWh-e/m3)": SEEC_cryst,"Cryst STEC (kWh-th/m3)": STEC_cryst,  "Pumping energy (kWh-e/m3)": pump_energy_brackish,
                            "NF CAPEX ($)": NF_capex, "MDB CAPEX ($)": MDB_capex, "Cryst CAPEX ($)": cryst_capex, "Drilling cost ($)": drilling_cost,
        ...
        latitude, longitude, brackish_tds, NF_permeate_tds, NF_brine_tds, depth, 
        brackish_avail, desal_cap, 
        cryst_cap, Thermal_req, grid_rate, from_EIA,
        ) = info
